...
public class Worker implements Runnable {
        ...
        for (HtmlElement element : elements) {
          ...
          if (titleTds != null && !titleTds.isEmpty()) {
            ...
            Optional<NewsType> optional = Arrays.stream(NewsType.values())
                .filter(x -> newsCategory.equals(x.getName())).findFirst();
            if (optional.isPresent()) {
              NewsType newsType = optional.get();
              HtmlElement titleElement = element.getElementsByAttribute("td", "class", "ntitle")
                  .get(0);
              ...
              if (newsElements != null && !newsElements.isEmpty()) {
                ...
                String newsTitle = ((HtmlElement) newsElement
                ...
                String newsGender = "";
                Optional<String> gender = lines.stream()
                    .filter(line -> StringUtils.containsIgnoreCase(line, "Стиль")).findFirst();
                if (gender.isPresent()) {
                ...
                if (newsGender.length() == 0) {
                  gender = lines.stream()
                      .filter(line -> StringUtils.containsIgnoreCase(line, "Жанр")).findFirst();
                  if (gender.isPresent()) {
                    newsGender += gender.get();
                  }
                }
                ...
                if (newsType != NewsType.Concerts) {
                  Optional<String> country = lines.stream()
                      .filter(line -> StringUtils.containsIgnoreCase(line, "Страна")).findFirst();
                  if (country.isPresent()) {
                    newsCountry += country.get();
                  }
                  if (newsCountry.length() == 0) {
                    country = lines.stream()
                        .filter(line -> StringUtils.containsIgnoreCase(line, "Родина")).findFirst();
                    if (country.isPresent()) {
                      newsCountry += country.get();
                    }
                  }
                  ...
                  Optional<String> format = lines.stream()
                      .filter(line -> StringUtils.containsIgnoreCase(line, "Формат")).findFirst();
                  if (format.isPresent()) {
                    newsFormat += format.get();
                  }
                  if (newsFormat.length() == 0) {
                    format = lines.stream()
                        .filter(line -> StringUtils.containsIgnoreCase(line, "Качество"))
                        .findFirst();
                    if (format.isPresent()) {
                      newsFormat += format.get();
                    }
                ...
                if (newsImageUrl == null || newsImageUrl.isEmpty() || !newsImageUrl
                    .contains("fastpic")
                ...
                News news = new News(newsTitle, newsType, dateTime, newsGender, newsFormat,
                    newsCountry, tracks, href, newsImageUrl);
  ...
  private String getTrackList(ArrayList<String> lines) {
    ...
    for (int i = 0; i < lines.size(); i++) {
      if (StringUtils.contains(lines.get(i), "Треклист") || StringUtils
          .contains(lines.get(i), "Tracklist")) {
